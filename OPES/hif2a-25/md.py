...
a = traj.topology.select("(resname L01) and (name C7)")
b = traj.topology.select("(resname L01) and (name C8)")
c = traj.topology.select("(resname L01) and (name O1)")
d = traj.topology.select("(resname L01) and (name C9)")
print(a[0], b[0], c[0], d[0])

plumed_script_1 = f"""
t1: TORSION ATOMS={a[0]+1},{b[0]+1},{c[0]+1},{d[0]+1}
opes: OPES_METAD ARG=t1 PACE=2500 BARRIER=20 TEMP=300
